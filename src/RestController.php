...
class RestController {
    ...
    private function saved_session_post_to_session( array $session_post ): array {
        return [
            ...
            'start_local'    => isset( $session_post['start_local'] ) ? sanitize_text_field( (string) $session_post['start_local'] ) : '',
            ...
            'end_local'      => isset( $session_post['end_local'] ) ? sanitize_text_field( (string) $session_post['end_local'] ) : '',
            'time_range'     => isset( $session_post['time_range'] ) ? sanitize_text_field( (string) $session_post['time_range'] ) : '',
    ...
    private function compact_session( array $session ): array {
        return [
            ...
            'start_local'    => isset( $session['start_local'] ) ? sanitize_text_field( (string) $session['start_local'] ) : '',
            'duration'       => isset( $session['duration'] ) ? absint( $session['duration'] ) : 0,
            'end'            => isset( $session['end'] ) ? absint( $session['end'] ) : null,
            'end_local'      => isset( $session['end_local'] ) ? sanitize_text_field( (string) $session['end_local'] ) : '',
            'time_range'     => isset( $session['time_range'] ) ? sanitize_text_field( (string) $session['time_range'] ) : '',
